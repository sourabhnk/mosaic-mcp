...
- tool-result cache: (tool_name, normalized_params + ``__tier``, kg_version)
- agent-response cache: (normalized_query, agent_version, tier, kg_version)

**Tier is part of both keys, and it is not optional.** Both layers store a
payload that has already been shaped to the answering user's tier, so a key
without tier hands a Pro-sized answer to a free user and a free user's
truncated answer to someone who paid for the full one. The agent layer
shipped without it; see ``tests/unit/test_agent_cache_tier.py``.

**A cache hit is metered.** It is a served answer, and the caller must
account for it exactly as it accounts for a miss — same tool name, same
target symbol, same limit checks. The original Op-6 note here said the
opposite ("cache reads are free to the user"), and under it a free user
could replay any cached question indefinitely: no ``usage_tracking`` row,
and the daily unique-target limit never advanced.

Every operation is fail-safe: any cache error degrades to a normal
(uncached) call. No PII enters cache keys (params/queries are normalised,
never user identifiers) — tier is a coarse class, not an identifier.
"""
...
def agent_cache_get(query: str, agent_version: str, tier: str) -> str | None:
    """``tier`` is required, deliberately.

    A default would let a caller that has not been updated keep the old
    cross-tier behaviour silently, and ``sync_pip_package.py`` copies this
    module without its callers.
    """
    key = _key(query.strip().lower(), agent_version, tier, _kg_version())
    hit = _get("agent_response_cache", key)
    if hit is not None:
        logger.info("cache_hit agent v=%s tier=%s", agent_version, tier)
        return hit if isinstance(hit, str) else json.dumps(hit)
    logger.info("cache_miss agent v=%s tier=%s", agent_version, tier)
    return None


def agent_cache_put(query: str, agent_version: str, tier: str, response: str) -> None:
    v = _kg_version()
    key = _key(query.strip().lower(), agent_version, tier, v)
    try:
        _db().execute(
            """
